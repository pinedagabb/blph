...
<body>	
    ...
    <div class = "container postboxcontainer">
        <?php 
				$result = mysqli_query($mysqli, "SELECT * FROM bucketlist WHERE username='".$_SESSION['username']."'");
				if(mysqli_num_rows($result)>0){
                    ...
                    echo "<div class = 'pull-right'>
                            ...
                            <a onclick = 'likePost(".$data[0].")' style = 'cursor:pointer;'><img id = 'like".$data[0]."'src= 'img/postbox/heart.png' class = 'ico'></a><span id = 'likecnt".$data[0]."'class = 'icotxt'>".$data[8]."</span>";
                    echo    "<a onclick = 'addPost(".$data[0].")' style = 'cursor:pointer;'><img id = 'add".$data[0]."'src= 'img/postbox/minus.png' class = 'ico'></a>";
                    echo   "</div>";
                    echo "</div>";
				}
                }else{
                    echo "<center><p class = 'emptypromptsmall' style = 'padding-top:50px;'>You have not added anything in your bucketlist :-(</p></center>";
                }
        ?>
    </div>
    </div>
